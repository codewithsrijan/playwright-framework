...
var { timeOrigin } = require("playwright-core/lib/coreBundle").iso;
...
var { assert: assert2 } = require("playwright-core/lib/coreBundle").iso;
...
var { getPackageManagerExecCommand: getPackageManagerExecCommand2 } = require("playwright-core/lib/coreBundle").utils;
...
async function showHTMLReport(reportFolder, host = "localhost", port, testId) {
  ...
  try {
    assert2(import_fs5.default.statSync(folder).isDirectory());
  } catch (e) {
    writeLine(colors2.red(`No report found at "${folder}"`));
...
var { monotonicTime: monotonicTime4 } = require("playwright-core/lib/coreBundle").iso;
...
async function detectChangedTestFiles(baseCommit, configDir) {
  function gitFileList(command) {
    try {
      return import_child_process2.default.execSync(
        `git ${command}`,
      ...
      if (unknownRevision) {
        const isShallowClone = import_child_process2.default.execSync("git rev-parse --is-shallow-repository", { encoding: "utf-8", stdio: "pipe", cwd: configDir }).trim() === "true";
        if (isShallowClone) {
          throw new Error([
...
async function runTasks(testRun, tasks, globalTimeout, cancelPromise) {
  const deadline = globalTimeout ? monotonicTime5() + globalTimeout : 0;
...
var AttachmentPathPatcher = class {
  ...
  _patchAttachments(attachments) {
    for (const attachment of attachments) {
      if (!attachment.path)
        continue;
      attachment.path = import_path19.default.join(this._resourceDir, attachment.path);
    }
  }
};
