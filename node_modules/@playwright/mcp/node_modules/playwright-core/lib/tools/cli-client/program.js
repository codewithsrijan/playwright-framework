...
async function program(options) {
  ...
  if (args.help || args.h || !commandName) {
    if (command) {
      ...
      if ((0, import_utils.isCodingAgent)())
  ...
  switch (commandName) {
    ...
    case "show": {
      ...
      if (args.annotate) {
        const dashboard = (0, import_child_process.spawn)(process.execPath, daemonArgs, { detached: true, stdio: "ignore" });
        dashboard.unref();
        const annotate = (0, import_child_process.spawn)(process.execPath, [...daemonArgs, "--annotate"], { stdio: "inherit" });
        await new Promise((resolve) => annotate.on("exit", () => resolve()));
        return;
      }
