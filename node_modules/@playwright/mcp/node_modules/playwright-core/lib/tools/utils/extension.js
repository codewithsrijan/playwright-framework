...
const playwrightExtensionInstallUrl = `https://chromewebstore.google.com/detail/playwright-mcp-bridge/${playwrightExtensionId}`;
async function isPlaywrightExtensionInstalled(userDataDir) {
  if (await pathExists(import_path.default.join(userDataDir, "Default", "Extensions", playwrightExtensionId)))
    return true;
  try {
    const prefs = await import_fs.default.promises.readFile(import_path.default.join(userDataDir, "Default", "Preferences"), "utf-8");
